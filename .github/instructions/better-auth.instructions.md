...
const authMiddleware: Middleware = () => {
    return async (ctx, next) => {
        ...
        if (ctx.data.betterAuthSetCookie) {
            ctx.res.headers ||= {};
            let origSetCookie =
                ctx.res.headers['set-cookie'] || ([] as string[]);
        ...
        organization({
            organizationHooks: {
                ...
                beforeUpdateMemberRole: async ({
                    ...
                    if (
                        newRole === 'owner' &&
                        !hasOwnerUpgradePermission(user)
                    ) {
        ...
        organization({
            organizationHooks: {
                ...
                afterRejectInvitation: async ({
                    ...
                    await notifyInviterOfRejection(
                ...
                afterCancelInvitation: async ({
                    ...
                    await logInvitationCancellation(
                        invitation.id,
                        cancelledBy.id
                    );
                }
        ...
        organization({
            organizationHooks: {
                ...
                beforeCreateTeam: async ({ team, user, organization }) => {
                    ...
                    const existingTeam = await findTeamByName(
                        team.name,
                        organization.id
                    );
                    if (existingTeam) {
        ...
        sso({
            defaultSSO: {
                ...
                samlConfig: {
                    ...
                    cert: '-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----',
                    callbackUrl:
                        'http://localhost:3000/api/auth/sso/saml2/sp/acs',
                    spMetadata: {
                        entityID:
