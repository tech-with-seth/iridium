 ...
 */
export function isFavorited({ userId, postId }: { userId: string; postId: string }) {
 ...
 */
export function addFavorite({ userId, postId }: { userId: string; postId: string }) {
 ...
 */
export function removeFavorite({ userId, postId }: { userId: string; postId: string }) {
 ...
 */
export async function toggleFavorite({ userId, postId }: { userId: string; postId: string }) {
...
export function getUserFavorites({
  ...
  return prisma.favorite.findMany({
    ...
    orderBy: sortBy === 'createdAt' ? { createdAt: 'desc' } : { post: { title: 'asc' } },
    take: limit,
    skip: offset,
  });
}
...
import { toggleFavoriteSchema, type ToggleFavoriteData } from '~/lib/validations';
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (request.method === 'POST') {
    const { data: validatedData, errors } = await getValidatedFormData<ToggleFavoriteData>(
      request,
    ...
    try {
      ...
      return data({ error: 'Failed to update favorite status' }, { status: 500 });
    }
  }
